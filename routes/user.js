...
router.get('/edit', async (req, res) => {
  ...
  if (!userInfo) return res.render('user/error', {
    message: 'User not The user does not exist'
  });
...
router.get('/content', function (req, res) {
  ...
  Content.where({ user: req.userInfo._id }).countDocuments().then(function (data) {
    count = data;
    console.log("aggregate data：", count);
...
router.post('/content/add', function (req, res) {
  ...
  if (req.body.title === '') {
    res.render('user/error', {
      userInfo: req.userInfo,
      message: 'Blog titles cannot be empty',
    });
    return;
  }
...
router.get('/content/edit', function (req, res) {
  ...
  Category.find().then(function (data) {
    ...
    if (!data) {
      res.render('user/error', {
        userInfo: req.userInfo,//当前用户
        message: 'Blogpost information does not exist'
      });
